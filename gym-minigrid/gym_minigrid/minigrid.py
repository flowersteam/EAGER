...
class Grid:
    ...
    def render_tile(
        ...
        img = np.ones(shape=(tile_size * subdivs, tile_size * subdivs, 3), dtype=np.uint8) * int(200)
    ...
    def render_tile_traj(
        ...
        value = min(int(255), int(10*traj_freq+140))
        # Hash map lookup key for the cache
        key = (agent_dir, highlight, tile_size, value)
        key = obj.encode() + key if obj else key
        ...
        tile = np.array([value, value, 0], dtype=np.uint8)
        img = np.tile(tile, reps=(tile_size * subdivs, tile_size * subdivs, 1))
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                if traj[i][j] == 0:
                    ...
                    tile_img = Grid.render_tile_traj(cell,
                                                     agent_dir=agent_dir if agent_here else None,
                                                     highlight=highlight_mask[i, j],
                                                     traj_freq=traj[i][j],
                                                     tile_size=tile_size
