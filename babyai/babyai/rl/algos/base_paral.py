...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
        ...
        self.log_success_rate_QA = [0] * self.num_procs
    ...
    def collect_experiences(self, debug=False):
        ...
        for i in tqdm(range(self.num_frames_per_proc), ascii=" "*9 + ">", ncols=100):
            ...
            if idr_torch.rank==0:
                ...
                obs, reward, done, env_info = self.env.step(None)
            if idr_torch.rank==0:
                QG_QA_reward = False
                if isinstance(env_info, tuple) and len(env_info) == 2:
                    ...
                    success_rate_QA = env_info["success_rate_QA"]
                else:
                    info = env_info
                if self.aux_info:
                ...
                for i, done_ in enumerate(done):
                    if done_:
                        ...
                        if QG_QA_reward:
                            self.log_success_rate_QA.append(success_rate_QA[i])
        ...
        if idr_torch.rank==0:
            ...
            if QG_QA_reward:
                log = {
                    ...
                    "success_rate_QA": self.log_success_rate_QA[-keep:]
                }
